...
router.get('/categories', authenticate, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('books').select('category').not('category', 'is', null).order('category');
    if (error) return res.status(500).json({ message: error.message });
...
router.post('/', authenticate, requireMinRole('admin'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('books')
      .insert({ title, author, isbn: isbn||null, category: category||null,
...
router.put('/:id', authenticate, requireMinRole('admin'), async (req, res) => {
  try {
    ...
    const newAvailable = Math.max(0, book.available_copies + (newTotal - book.total_copies));
...
router.delete('/:id', authenticate, requireMinRole('admin'), async (req, res) => {
  try {
    const { count } = await supabase.from('transactions')
      ...
      .eq('book_id', req.params.id).in('status', ['active', 'pending']);
